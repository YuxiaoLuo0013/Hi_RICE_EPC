...
class EpidemicSimulation:
    ...
    COMP_NAMES: [str] = ["SUSC", "LATT", "INCB", "ASYM", "SYMP", "MILD", "SEVE", "HOSP", "ICU", "HOSPR", "RECV",
                         "DEATH", "PROT", "OUT"]
    ...
    def __init__(
            ...
            seed: int = 1,
            path_param = r"/EPC_RF/env/EPC_env/Parameter/",
            path_prop = r"/EPC_RF/env/EPC_env/Property/",
            path_data = r"/EPC_RF/env/EPC_env/Data/",
    ) -> None:
    ...
    def set_PHSM_params(self):
        ...
        r_ic_other, r_ic_work, is_npi_gather, is_school_off, is_iso_h, rate_iso_p_work_off = self.DICT_PHSM_PARAMS[
            self.temp_level]
    ...
    def _set_contact(self):
        ...
        self.ic_setting = {self.HOME: 0.37, self.KINDER: 0.25, self.PRIMARY: 0.25, self.MIDDLE: 0.25, self.HIGH: 0.25,
                           self.WORK: 0.26, self.OTHER: 0.1}
    ...
    def _set_trans_rate(self):
        ...
        """
        trans_rate_v0 = pickle.load(open(os.path.join(self.path_param, "Epidemiological/", "TransitionRate-Adj.pkl"),
                                         "rb"))  # <-----------------
        vaccine_efficacy = pd.read_excel(os.path.join(self.path_param, "Ref Parameter/VE-conditional-统一参数.xls"),
                                         index_col = "dose")  # <-----------------
    ...
    def _set_vaccine_efficacy_waning(self, trans_rate_v0, vaccine_efficacy, method = "linear"):
        ...
        for dose, row in vaccine_efficacy.iterrows():
            vaccine_efficacy_symp = np.r_[
                np.linspace(row.ve_symp, row.min_ve_symp, int(row.span)), np.full(int(self.max_VE_span - int(row.span)),
                                                                                  row.min_ve_symp)]
            vaccine_efficacy_hosp = np.r_[
                np.linspace(row.ve_hosp, row.min_ve_hosp, int(row.span)), np.full(int(self.max_VE_span - int(row.span)),
                                                                                  row.min_ve_hosp)]
            vaccine_efficacy_death = np.r_[np.linspace(row.ve_death, row.min_ve_death, int(row.span)), np.full(
                int(self.max_VE_span - int(row.span)), row.min_ve_death)]
            vaccine_efficacy_inf = np.r_[np.linspace(row.ve_inf, row.min_ve_inf, int(row.span_inf)), np.full(
                int(self.max_VE_span - int(row.span)), row.min_ve_inf)]
            vaccine_efficacy_inf = np.pad(vaccine_efficacy_inf, (0, int(self.max_VE_span - row.span_inf)),
                                          mode = "edge")
    ...
    def _plot_vaccine_efficacy_waning(self, method = "linear"):
        ...
        vaccine_efficacy = pd.read_excel(os.path.join(self.path_param, "Ref Parameter/VE setting.xls"),
                                         index_col = "dose")
        ...
        ve = defaultdict(dict)
        for dose, row in vaccine_efficacy.iterrows():
            ve["VE against infection"][dose] = np.pad(np.linspace(row.ve_inf_, row.min_ve_inf, int(row.span)),
                                                      (0, int(self.max_iterday - row.span)), "edge")
            ve["VE against symptomatic"][dose] = np.pad(np.linspace(row.ve_symp, row.min_ve_symp, int(row.span)),
                                                        (0, int(self.max_iterday - row.span)), "edge")
            ve["VE against hospitalization"][dose] = np.pad(np.linspace(row.ve_hosp, row.min_ve_hosp, int(row.span)),
                                                            (0, int(self.max_iterday - row.span)), "edge")
            ve["VE against mortality      "][dose] = np.pad(np.linspace(row.ve_death, row.min_ve_death, int(row.span)),
                                                            (0, int(self.max_iterday - row.span)), "edge")
    ...
    def _set_compartment_duration(self):
        ...
        是否能够二次感染
        """
        self.compart_duration = \
        pd.read_excel(os.path.join(self.path_param, "Ref Parameter/compartment duration-Adj.xls"),
                      index_col = "Endpoint").to_dict()["Duration"]  # <--------
    ...
    def _check_parameters_input(self):
        ...
        assert self.uptake_scenario in ["Uptake 00", "Uptake Current", "Uptake 90", "Uptake Enhence",
                                        "Uptake Enhence 60", "Uptake Enhence Homo",
                                        "Uptake Enhence 60 Homo"], "Wrong Input of Uptake Scenario!"
        assert bool(len(self.list_imported_daily)) ^ bool(
            self.n_imported_day0), "Either Input the List or the Number of Imported Seeds!"
    ...
    def _fast_load_structured_array(self, fn_person):
        ...
        self.tab_person_rank = tab_person_df[
            ['pid', 'hid', 'age', 'hzone', 'wzone', 'wtype', 'wplace', 'wgroup', "ranked_wplace", "ranked_wgroup",
             "ranked_hid", "ranked_hzone"]]
    ...
    def _load_agents(self):
        ...
        """
        fn_person = "".join([self.city_name] + ["/synpop"] + ["_spatial" if self.is_spatial else "_nonspatial"] + [
            "_samp_%0.2f" % (self.samp_rate) if self.is_samp else ""] + ["_new.csv"])
        fn_hdict = "".join([self.city_name] + ["/colocation"] + ["_spatial" if self.is_spatial else "_nonspatial"] + [
            "_samp_%0.2f" % (self.samp_rate) if self.is_samp else ""] + ["_hdict.pkl"])
        fn_hzdict = "".join([self.city_name] + ["/colocation"] + ["_spatial" if self.is_spatial else "_nonspatial"] + [
            "_samp_%0.2f" % (self.samp_rate) if self.is_samp else ""] + ["_hzdict.pkl"])
        fn_hjdict = "".join([self.city_name] + ["/colocation"] + ["_spatial" if self.is_spatial else "_nonspatial"] + [
            "_samp_%0.2f" % (self.samp_rate) if self.is_samp else ""] + ["_hjdict.pkl"])
        fn_wpdict = "".join([self.city_name] + ["/colocation"] + ["_spatial" if self.is_spatial else "_nonspatial"] + [
            "_samp_%0.2f" % (self.samp_rate) if self.is_samp else ""] + ["_wpdict.pkl"])
        fn_wgdict = "".join([self.city_name] + ["/colocation"] + ["_spatial" if self.is_spatial else "_nonspatial"] + [
            "_samp_%0.2f" % (self.samp_rate) if self.is_samp else ""] + ["_wgdict.pkl"])
        ...
        self.pid_students = \
        np.where(np.isin(self.tab_person["wtype"], [self.KINDER, self.PRIMARY, self.MIDDLE, self.HIGH]))[0]
    ...
    def __get_metadata__(self):
        ...
        drop_keys = ["hdict", "hzdict", "wpdict", "wgdict", "tab_person", "trans_rate", "gdf_sq", "dict_qu_jd",
                     "dict_jd_sq"]
    ...
    def init_sim_mat(self):
        ...
        s_dtype = np.dtype([("pid", "int32"), ("comp_this", "int8"), ("comp_next", "int8"), ("cd_trans", "float"),
                            ("type_imune", "int8"), ("days_imune", "int16"), ("days_infect", "int16"),
                            ("type_quran", "int8"),
    ...
    def get_stat_features(self):
        ...
        self.wp_num = df_onehot.groupby(['wplace'])['pid'].transform('count').to_numpy()
        self.wg_num = df_onehot.groupby(['wgroup'])['pid'].transform('count').to_numpy()
        self.home_num = df_onehot.groupby(['hid'])['pid'].transform('count').to_numpy()
        self.com_num = df_onehot.groupby(['hzone'])['pid'].transform('count').to_numpy()
    ...
    def get_situation_features(self, sim_mat, sim_date):
        ...
        if sim_date == 0:
            self.increase_num = np.zeros((self.n_agent, 4, 12))
            self.prob_infecious = np.zeros((self.n_agent))
            self.com_lag = sim_mat['comp_this']
            self.control_state = np.zeros((3, self.n_agent, 4))
            dynamic_features_df['increase_infectons']=np.zeros(self.n_agent)

        elif sim_date > 0:

            self.control_state[0, :, sim_date % 4] = np.array(sim_mat['type_quran'] == self.Q_FREE).astype(float)
            self.control_state[1, :, sim_date % 4] = np.array(sim_mat['type_quran'] == self.Q_COM).astype(float)
            self.control_state[2, :, sim_date % 4] = np.array(sim_mat['type_quran'] == self.Q_ISO_HOME).astype(float)
		
            if sim_date - 4 < 0:
                if_out_com = np.sum(self.control_state[0, :, 1:sim_date + 1], axis = 1) > 0 
                if_com = if_out_com | (np.sum(self.control_state[1, :, 1:sim_date + 1], axis = 1) > 0)
                if_home = if_com | (np.sum(self.control_state[2, :, 1:sim_date + 1], axis = 1) > 0)
           

            elif sim_date - 4 >= 0:
                if_out_com = np.sum(self.control_state[0, :, :], axis = 1) > 0
                if_com = if_out_com | (np.sum(self.control_state[1, :, :], axis = 1) > 0)
                if_home = if_com | (np.sum(self.control_state[2, :, :], axis = 1) > 0)

                
            if_out_com=if_out_com.astype(float)
            if_com=if_com.astype(float)
            if_home=if_home.astype(float)
            
            increase_infectons = ((self.com_lag <= 3) & (sim_mat['comp_this'] > 3)).astype(float)
            dynamic_features_df['increase_infectons']=np.array(increase_infectons)
#            dynamic_features_df['increase_infectons_out_com'] = np.array(increase_infectons) 
#            dynamic_features_df['increase_infectons_com'] = np.array(increase_infectons) 
#            dynamic_features_df['increase_infectons_home'] = np.array(increase_infectons) 
            ...
            increase_wp = dynamic_features_df.groupby('wplace')['increase_infectons_out_com'].transform(
                'sum').to_numpy()
            increase_wp[self.tab_person['wtype'] == -1] = 0
            increase_wp = 1-increase_wp *  if_out_com/self.wp_num

            increase_wg = dynamic_features_df.groupby('wgroup')['increase_infectons_out_com'].transform(
                'sum').to_numpy()
            increase_wg[self.tab_person['wtype'] == -1] = 0
            increase_wg = 1-increase_wg*  if_out_com/self.wg_num
            ...
            increase_home = 1-increase_home * if_home/self.home_num

            increase_com = dynamic_features_df.groupby('hzone')['increase_infectons_com'].transform('sum').to_numpy()
            increase_com = 1-increase_com * if_com/self.com_num

            increase_num = np.c_[increase_wp, increase_wg, increase_home, increase_com]
            self.increase_num[:, :, (sim_date-1) % 12] = increase_num
#            if sim_date < 10:
#                self.prob_infecious = 1 - (
#                            1 - np.mean(self.increase_num[:, 0, :sim_date+1], axis = 1) / self.wp_num.reshape(-1)) * (
#                                                  1 - np.mean(self.increase_num[:, 1, :sim_date+1],
#                                                              axis = 1) / self.wg_num.reshape(-1)) * (
#                                                  1 - np.mean(self.increase_num[:, 2, :sim_date+1],
#                                                              axis = 1) / self.home_num.reshape(-1)) * (
#                                                  1 - np.mean(self.increase_num[:, 3, :sim_date+1],
#                                                              axis = 1) / self.com_num.reshape(-1))
#            elif sim_date >= 10:
#                self.prob_infecious = 1 - (1 - np.mean(self.increase_num[:, 0, :], axis = 1) / self.wp_num.reshape(-1)) * (
...
#                                              1 - np.mean(self.increase_num[:, 3, :], axis = 1) / self.com_num.reshape(-1))
            ...
            if sim_date < 12:
                self.prob_infecious = 1 - np.prod(self.increase_num[:, 0, :sim_date ] , axis = 1) \
                                      * np.prod(self.increase_num[:, 1, :sim_date ] , axis = 1) \
                                      * np.prod(self.increase_num[:, 2, :sim_date ] , axis = 1) \
                                      * np.prod(self.increase_num[:, 3, :sim_date ], axis = 1)

            elif sim_date >= 12:
                self.prob_infecious = 1 - np.prod(self.increase_num[:, 0, :] , axis = 1) \
                                      * np.prod(self.increase_num[:, 1, :] , axis = 1) \
                                      * np.prod(self.increase_num[:, 2, :] , axis = 1) \
                                      * np.prod(self.increase_num[:, 3, :] , axis = 1)
        ...
        self.stat_prob=self.stat_prob*(np.max(self.prob_infecious)+1e-3)
        ...
        other_column = ['prob>0', 'prob>0.1','increase_infectons']
        com_features = dynamic_features_df.groupby('ranked_hzone')[
            onehot_comp_columns + onehot_type_quran_columns + other_column].sum()
        ...
        global_features = dynamic_features_df[
            onehot_comp_columns + onehot_type_quran_columns + other_column].sum().to_numpy().reshape(1, -1).repeat(
            com_features.shape[0], axis = 0)
        self.features = np.c_[com_features, global_features]
        # self.prob_infecious = None
...
#        region_infected = dynamic_features_df.groupby('ranked_hzone')['infected'].sum().to_numpy()
        ...
        return self.features, prob, region_controlled
    ...
    def simulate_imported(self, sim_mat, sim_date):
        if sim_date == 0:  # 第0天 投入n_imported_day0数量随机种子 或种子列表
            if self.n_imported_day0 > 0:
                pid_imported_daily = np.random.choice(self.pid_unprotect, self.n_imported_day0,
                                                      replace = False)  # 采用随机种子
            else:
        ...
        if pid_imported_daily.size:  # 更新 sim_mat
            ...
            date_pid_imported_daily = np.c_[
                np.repeat(sim_date, n_imported), pid_imported_daily]  # 存储到 self.date_pid_imported
    ...
    def simulate_isolation_home_asym(self, sim_mat, pid_asym):
        ...
        pid_iso_h = np.concatenate(list(map(self.hdict.get, hid_iso_h))) if hid_iso_h.size else np.empty(shape = 0,
                                                                                                         dtype = int)
    ...
    def simulate_symp(self, sim_mat, sim_mat_new):
        ...
        pid_seve_0, pid_mild_0 = self.get_pid_with_outcome(sim_mat, pid_symp_drug, "symp2seve",
                                                           epsilon = self.e_drug) if pid_symp_drug.size else [np.empty(
            shape = 0, dtype = int)] * 2
        pid_seve_1, pid_mild_1 = self.get_pid_with_outcome(sim_mat, pid_symp_no_drug, "symp2seve",
                                                           epsilon = 0) if pid_symp_no_drug.size else [np.empty(
            shape = 0, dtype = int)] * 2
    ...
    def simulate_transition(self, sim_mat):
        ...
        assert not np.where(sim_mat["comp_this"] == self.SYMP)[
            0].size, "Error Occur when Processing Compartment Transition: Compart=SYMP!"
    ...
    def lookup_contact_other(self, pid_infectious, hzdict, place, stype):
        ...
        pid_nco_ozones = pd.DataFrame(self.tab_person[pid_infectious][["pid", "h_sq", "nco_sq"]],
                                      columns = ["pid", "h_sq", "nco_sq"])
        ...
        targets = map(
            lambda x: np.random.choice(hzdict.get(x[0]), x[1]) if x[0] in hzdict.keys() else np.repeat(-1, x[1]).astype(
                int), ozone_nco)
    ...
    def lookup_contact_other2(self, pid_infectious, hjdict, place, stype):
        ...
        pid_nco_ozones = pd.DataFrame(self.tab_person[pid_infectious][["pid", "h_jd", "nco_jd"]],
                                      columns = ["pid", "h_jd", "nco_jd"])
        # pid_nco_ozones["nco_sq"] = pid_nco_ozones["nco_sq"].mask(pid_nco_ozones["nco_sq"]>self.npi_max_gather, self.npi_max_gather)               # <----------------
        ...
        targets = map(
            lambda x: np.random.choice(hjdict.get(x[0]), x[1]) if x[0] in hjdict.keys() else np.repeat(-1, x[1]).astype(
                int), ozone_nco)
        targets = np.concatenate(list(targets))
    ...
    def get_infection_other(self, pid_infectious, source_type):
        ...
        prob_trans = self.ptrans * self.ic_setting[self.OTHER] * self.infectivity[
            source_type] * self.r_ic_other  # <--------------
    ...
    def get_infection_by_source_type(self, sim_mat, pid_infectious, source_type):
        ...
        infections_other = infections_other[(sim_mat['type_quran'][infections_other[:, 1]] == self.Q_FREE) | (
                    sim_mat['type_quran'][infections_other[:, 1]] == self.Q_COM)]

        infections_other2 = self.get_infection_other2(pid_infectious_q_free, source_type = source_type)
        infections_other2 = infections_other2[(sim_mat['type_quran'][infections_other2[:, 1]] == self.Q_FREE) | (
                    sim_mat['type_quran'][infections_other2[:, 1]] == self.Q_COM)]

        infections = np.concatenate(
            [infections_home, infections_iso, infections_occup, infections_other, infections_other2])
        return infections
    ...
    def get_infection(self, sim_mat, sim_date):
        ...
        if not infections.size:
            return np.empty(shape = (0, 6), dtype = int), np.empty(shape = 0, dtype = int), np.empty(shape = (0, 4),
                                                                                                     dtype = int)
    ...
    def process_sim_result(self, sim_res):
        ...
        if self.is_fast_r0:
            ...
            test_duration = self.make_duration_lognorm("latt2incb", self.n_agent) + self.make_duration_lognorm(
                "incb2symp", self.n_agent)
            s_days, e_days = int(min(test_duration)), int(max(test_duration)) + 1

            date_onset_s = np.fromiter(
                map(lambda x: self.get_date_onset(x[0], x[1], pid_onset_concat, n_daily_onset_cusm, s_days, e_days),
                    zip(sources, date_inf_s)), dtype = int)
            date_onset_t = np.fromiter(
                map(lambda x: self.get_date_onset(x[0], x[1], pid_onset_concat, n_daily_onset_cusm, s_days, e_days),
                    zip(targets, date_inf_t)), dtype = int)
        ...
        inf_dtype = np.dtype([("date_inf_t", "int16"), ("sources", "int32"), ("targets", "int32"), ("places", "int8"),
                              ("stypes", "int8"), ("GT", "int16"), ("date_inf_s", "int16"), ("date_onset_s", "int16"),
                              ("date_onset_t", "int32")])
    ...
    def init_sim_res(self, sim_mat):
        ...
        sim_res["infections"] = np.empty(shape = (0, 6),
                                         dtype = int)  # infections 在追溯密接时用到, 需要先进行结构化, 不能在最后处理sim_res时concatenate
    ...
    def update_sim_result_statistic(self, sim_res):
        ...
        infection_onset = sim_res["infections"][
            (sim_res["infections"]["date_onset_s"] > 0) * (sim_res["infections"]["date_onset_t"] > 0)]
    ...
    def update_sim_result_daily(self, sim_res, sim_mat, pid_trans, daily_infections, pid_blocked, wg_close):
        ...
        daily_counter[self.LATT] = daily_infections.shape[
                                       0] + self.n_imported_daily  # 其它仓室在转移时间为0时变化,但LATT由infection触发,需要单独记录
...
#        sim_res = self.init_sim_res(sim_mat)
        ...
        features, prob, region_controlled = self.get_situation_features(sim_mat, self.sim_date)
        self.region_num = self.tab_person_rank.groupby('ranked_hzone').size().to_numpy()

#        self.region_infected = np.zeros((self.max_iterday + 1, region_infected.shape[0]))
#        self.average_infected = np.zeros(self.max_iterday + 1)
        region_infected=np.zeros(region_controlled.shape[0])
        return features[0,:].reshape(1,-1), prob, sim_mat,  region_infected, region_controlled

    def step(self, sim_mat, action):
        self.sim_date += 1
...
#        self.average_infected[self.sim_date] = np.sum(region_infected) / infected_num_t
        ...
        return sim_mat, features[0,:].reshape(1,-1), prob, region_infected, region_controlled
...
if __name__ == '__main__':
    ES = EpidemicSimulation(city_name = "Shenzhen",
                            ptrans = 0.24, uptake_scenario = "Uptake 00",
                            ...
                            max_iterday = 30, n_imported_day0 = 100)
