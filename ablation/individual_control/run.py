...
register(
    ...
    entry_point = 'env.env_individual:EpidemicModel',  # 替换为您的环境类路径
...
if __name__ == "__main__":
    ...
    np.save(f"{daily_record_dir}/{p_trans}_daily_I.npy", daily_I[1:,:])
    np.save(f"{daily_record_dir}/{p_trans}_daily_Q.npy", daily_Q[1:,:])
    np.save(f"{daily_record_dir}/{p_trans}_Region_Q.npy", Region_Q_log[1:,:])
    print(f"Saved daily records to {daily_record_dir}")
