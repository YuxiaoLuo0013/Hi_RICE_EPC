...
def parse_args():
    ...
    parser.add_argument("--num-pop", type = int, default = 1346514,
...
def make_env(gym_id, test,p_trans, seed):
    def thunk():
...
if __name__ == "__main__":
    args = parse_args()
    run_name = f"{args.gym_id}__{args.exp_name}__{args.seed}__{int(time.time())}_{args.p_trans}"
    if args.track:
        import wandb
...
#    torch.use_deterministic_algorithms(True)
    ...
    device = torch.device("cuda:2" if torch.cuda.is_available() and args.use_gpu else "cpu")
    if torch.cuda.is_available():
        torch.cuda.manual_seed_all(args.seed)
    # env setup
    envs = gym.vector.AsyncVectorEnv(
        [make_env(args.gym_id, False,args.p_trans, args.seed + int(i * args.total_timesteps / args.num_envs) + args.num_envs) for i
         in range(args.num_envs)]
    )
    envs_test = gym.vector.AsyncVectorEnv(
       [make_env(args.gym_id,True, args.p_trans,args.test_seed + int(i + args.num_test)) for i in range(args.num_test)])
    test_score=1e6
    # reward_scaling = RewardScaling(shape = args.num_envs, gamma = args.gamma)
    Normalization = Normalization(shape = (587, 22))
    ...
    agent = c_Agent_lstm(envs, hidden_size = 512).to(device)
...
#        b_values = values.reshape(-1)
    ...
    # env setup
    envs = gym.vector.AsyncVectorEnv(
        [make_env('EpidemicModel-v0', True,args.p_trans, seed + int(i + num_envs)) for i in range(num_envs)]
    )
    # assert isinstance(envs.single_action_space, gym.spaces.Discrete), "only discrete action space is supported"
    # print sucessfully initialized num_envs env
    # envs = gym.wrappers.NormalizeObservation(envs)
