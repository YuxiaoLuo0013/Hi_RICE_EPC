...
def make_env(gym_id, test, p_trans, alpha,wta,wtp, seed):
    def thunk():
        env = gym.make(gym_id, if_test = test, seed = seed, autoreset = False, ptrans = p_trans, alpha = alpha,wta=wta,wtp=wtp)
...
if __name__ == "__main__":
    ...
    envs_test = gym.vector.AsyncVectorEnv(
        [make_env(args.gym_id, True, args.p_trans, args.alpha,args.wta,args.wtp,args.test_seed + int(i + args.num_test)) for i in
         range(args.num_test)])
    test_score = 1e9
