...
class EpidemicSimulation:
    ...
    def get_stat_features(self):
        ...
        self.stat_prob=self.stat_prob+self.home_num.reshape(-1)+np.mean(self.tab_df['nco_sq'].to_numpy())+np.mean(self.tab_df['nco_jd'].to_numpy())
    ...
    def get_situation_features(self,sim_mat,sim_date):
        ...
        self.hea_prob=self.hea_prob*(1-home_symp_num*self.ptrans*self.ic_setting[self.HOME])\
                      *(1- (wp_symp_num+wg_symp_num)*self.ptrans*self.ic_setting[self.WORK]/(self.wp_num.reshape(-1)+self.wg_num.reshape(-1))\
                      -com_symp_num*self.ptrans*self.ic_setting[self.OTHER]/self.com_num.reshape(-1))

        self.infect_prob=1-self.hea_prob
        prob=np.c_[self.infect_prob,self.stat_prob]
        ...
        return individual_state,region_state,prob, region_controlled
