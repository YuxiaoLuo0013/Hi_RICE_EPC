...
class EpidemicSimulation:
    ...
    def get_situation_features(self,sim_mat,sim_date):
        ...
        total_infect=wp_symp_num+wg_symp_num+home_symp_num+com_symp_num
        total_people=self.wp_num+self.wg_num+self.home_num+self.com_num
        total_people=total_people.reshape(-1)

        self.hea_prob[(sim_mat['comp_this'] > 3) & (sim_mat['comp_this'] < 7)]=0

        heal_index=np.where((sim_mat['comp_this'] <= 3)|(sim_mat['comp_this'] >= 7))[0]
        self.hea_prob[heal_index]=self.hea_prob[heal_index]*(1-(total_infect[heal_index]/total_people[heal_index])*self.ptrans)
        self.infect_prob=1-self.hea_prob

        region_infect=dynamic_features_df.groupby('ranked_hzone')['comp_1'].sum().to_numpy()
        region_num=dynamic_features_df.groupby('ranked_hzone')['pid'].count().to_numpy()
        region_health=region_num-region_infect
        dynamic_features_df['health_prob']=self.hea_prob*np.array((sim_mat['comp_this'] <= 3) | (sim_mat['comp_this'] >= 7)).astype(np.float32)
        health_factor=dynamic_features_df.groupby('ranked_hzone')['health_prob'].sum().to_numpy()/region_health

        region_state=np.c_[region_infect/region_num,health_factor]
        individual_state=np.c_[sim_mat_new['comp_this'],sim_mat_new['type_quran'],self.hea_prob,np.mean(sim_mat_new['comp_this']==1).repeat(self.n_agent),
        np.mean(sim_mat_new['type_quran']==1).repeat(self.n_agent),np.mean(sim_mat_new['type_quran']==2).repeat(self.n_agent),
        np.mean(sim_mat_new['type_quran']==3).repeat(self.n_agent),np.mean(sim_mat_new['type_quran']==4).repeat(self.n_agent),
        self.stat_prob,self.home_num,np.zeros((self.n_agent,1))]
    ...
    def get_infection_by_source_type(self, sim_mat, pid_infectious, source_type):
        ...
        infections_home  = self.get_infection_home (pid_infectious_q_free, source_type=source_type, iso=False)
...
if __name__=='__main__':
    ES = EpidemicSimulation(city_name="Shenzhen",
                            ptrans=0.10, uptake_scenario="Uptake 00",
                            ...
                            max_iterday=30, n_imported_day0=100,n_imported_daily=1)
    ...
    if num_elements>0:
        selected_elements = np.random.choice(control_index, size=num_elements, replace=False)
        individual_action_real[selected_elements]=0
